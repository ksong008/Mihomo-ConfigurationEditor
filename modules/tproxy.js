(function (window) {
    ...
    window.MihomoFeatureModules.createTproxyModule = function (ctx) {
        ...
        const tproxyBuilders = window.MihomoFeatureModules && window.MihomoFeatureModules.TproxyBuilders;
        if (!tproxyBuilders) {
            throw new Error('TproxyBuilders 未加载，请确认先引入 ./modules/tproxy-builders.js');
        }
        const {
        ...
        const routingCommands = computed(() => {
            return buildRoutingCommands(uiState.value.nftablesConfig || {});
        });
        ...
        const systemdService = computed(() => {
            return buildSystemdService(uiState.value.nftablesConfig || {});
        });

        const cleanNftablesScript = computed(() => {
            return buildCleanNftablesScript({
                nft: uiState.value.nftablesConfig || {},
                config: config.value
            });
        });
...
`;
        ...
        const installScript = computed(() => {
            return buildInstallScript(getCleanNftables(), systemdService.value);
        });
